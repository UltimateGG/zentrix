...
const ChipStyle = styled.div.attrs((props: ChipProps) => props)`
  ...
  background-color: ${props => {
    ...
    if (!props.selected)
      return props.theme.colors.background[2];

    const col = hexToRgb(props.theme.colors[props.variant || 'primary'][0]);
    return `rgba(${col.r}, ${col.g}, ${col.b}, 0.1)`;
  }};
  border: 2px solid ${props => props.theme.colors[props.selected ? (props.variant || 'primary') : 'background'][5]};
  ...
  &:hover {
    background-color: ${props => {
      ...
      if (!props.selected)
        return props.theme.colors.background[3];
...
const DeleteIconStyle = styled(Icon).attrs((props: ChipProps) => props)`
  ...
  &:hover {
    ...
    & path {
     fill: ${props => props.theme.colors.background[9]};
    }
  }
`;
