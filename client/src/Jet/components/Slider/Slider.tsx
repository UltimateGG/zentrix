...
const SliderStyle = styled.input.attrs((props: SliderProps) => props)`
  ...
  border: none;
  background-color: ${props => props.backgroundColor ? props.backgroundColor : props.theme.colors.background[3]};
  ...
  &::-webkit-slider-thumb {
    ...
    height: 1.4rem;
    border-radius: 50%;
    background-color: ${props => props.theme.colors[props.color || 'primary'][0]};
    border: 2px solid ${props => props.theme.colors[props.color || 'primary'][0]};
    cursor: ${props => (props.disabled ? 'not-allowed' : 'pointer')};
    transition: all 0.2s ease-in-out;

    ${props => !props.disabled && `
      &:hover {
        background-color: ${props.theme.colors[props.color || 'primary'][1]};
        border: 2px solid ${props.theme.colors[props.color || 'primary'][1]};
      }

      &:active {
        background-color: ${props.theme.colors[props.color || 'primary'][2]};
        border: 2px solid ${props.theme.colors.background[1]};
      }
    `}
  }
`;
...
const MinMaxStyle = styled.span.attrs((props: SliderProps) => props)`
  margin-bottom: 0;
  color: ${props => props.disabled ? props.theme.colors.background[5] : props.theme.colors.text[0]};
`;
...
const ValueStyle = styled.div.attrs((props: SliderProps) => props)`
  ...
  &::after {
    ...
    border-top: 3px solid ${props => props.theme.colors.background[5]};
    position: absolute;
    top: calc(100% + 2px);
    left: calc(50% - 3px); 
    right: 0;
