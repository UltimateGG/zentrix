...
const RadioButtonStyle = styled.input.attrs((props: RadioButtonProps) => props)`
  ...
  border-radius: 50%;
  color: ${props => props.theme.colors.background[4]};
  margin: 0.2rem 0;
  padding: 0.6rem;
  ...
  &:checked {
    background-color: rgba(${props => {
      const col = hexToRgb(props.theme.colors.primary[0]);
      return `${col.r}, ${col.g}, ${col.b}`;
    }}, 0.1);
    border: 2px solid ${props => props.theme.colors.primary[0]};
    ...
    &::after {
      ...
      width: 0.4rem;
      height: 0.4rem;
      background-color: ${props => props.theme.colors.text[7]};
      border-radius: 50%;
  ...
  &:not([disabled]):hover {
    ${props => props.checked ? '' : `background-color: ${props.theme.colors.background[3]};`}
  }
